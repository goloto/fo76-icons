...
export const readIcons = async (categoryOrder: CategoryOrder[]): Promise<Record<string, string[]>> => {
  const paths = await categoryOrder.reduce(async (accumulator, category) => {
    const awaitedAccumulator = await accumulator;
    const categoryIconPaths = await readdir(`${ICONS_DIRECTORY}/${category.name}`);
